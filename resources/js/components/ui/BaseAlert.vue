<template>
  <div :class="['rounded-xl border p-4 shadow-sm transition-all duration-300', alertClasses]">
    <div class="flex items-start gap-3">
      <div class="mt-0.5 shrink-0">
        <svg v-if="type === 'success'" class="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path>
        </svg>
        <svg v-else-if="type === 'error'" class="w-5 h-5 text-red-500" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z" clip-rule="evenodd"></path>
        </svg>
        <svg v-else-if="type === 'warning'" class="w-5 h-5 text-amber-500" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd"></path>
        </svg>
        <svg v-else class="w-5 h-5 text-blue-500" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z" clip-rule="evenodd"></path>
        </svg>
      </div>
      <div class="flex-1 w-full">
        <h3 v-if="title" class="text-sm font-semibold mb-1" :class="titleClasses">{{ title }}</h3>
        <p class="text-sm/relaxed" :class="messageClasses">{{ message }}</p>
      </div>
      <button
        v-if="closeable"
        @click="$emit('close')"
        type="button"
        class="inline-flex shrink-0 p-1.5 rounded-lg focus:ring-2 transition-colors opacity-70 hover:opacity-100"
        :class="buttonClasses"
        aria-label="Dismiss"
      >
        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
          <path fill-rule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clip-rule="evenodd"></path>
        </svg>
      </button>
    </div>
  </div>
</template>

<script setup lang="ts">
import { computed } from 'vue'

interface Props {
  type?: 'success' | 'error' | 'warning' | 'info'
  message: string
  title?: string
  closeable?: boolean
}

const props = withDefaults(defineProps<Props>(), {
  type: 'info',
  closeable: true,
})

const alertClasses = computed(() => {
  const classMap: Record<string, string> = {
    success: 'bg-green-50/50 border-green-200/60',
    error: 'bg-red-50/50 border-red-200/60',
    warning: 'bg-amber-50/50 border-amber-200/60',
    info: 'bg-blue-50/50 border-blue-200/60',
  }
  return classMap[props.type]
})

const titleClasses = computed(() => {
  const classMap: Record<string, string> = {
    success: 'text-green-800',
    error: 'text-red-800',
    warning: 'text-amber-800',
    info: 'text-blue-800',
  }
  return classMap[props.type]
})

const messageClasses = computed(() => {
  const classMap: Record<string, string> = {
    success: 'text-green-700',
    error: 'text-red-700',
    warning: 'text-amber-700',
    info: 'text-blue-700',
  }
  return classMap[props.type]
})

const buttonClasses = computed(() => {
  const classMap: Record<string, string> = {
    success: 'text-green-600 hover:bg-green-100 focus:ring-green-400',
    error: 'text-red-600 hover:bg-red-100 focus:ring-red-400',
    warning: 'text-amber-600 hover:bg-amber-100 focus:ring-amber-400',
    info: 'text-blue-600 hover:bg-blue-100 focus:ring-blue-400',
  }
  return classMap[props.type]
})

defineEmits<{
  close: []
}>()
</script>
